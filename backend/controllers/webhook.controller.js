...
const handleAirtableWebhook = async (req, res, next) => {
    try {
        ...
        if (!base || !base.id || !webhook || !webhook.id) {
            console.error("⚠️ Webhook: Missing base or webhook ID in request");
        ...
        if (!form) {
            console.error(`⚠️ Webhook: Form not found for webhook: ${webhookId}`);
        ...
        if (!user || !user.accessToken) {
            console.error(`⚠️ Webhook: User or access token not found for form: ${form._id}`);
        ...
        try {
            ...
            console.error(`❌ Webhook: Error fetching payloads:`, error.message);
            // Still return 200 to avoid Airtable retries
        ...
        console.log(`📬 Webhook: Processing ${payloads.payloads.length} payload(s)`);
